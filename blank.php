...
$user_data = check_login($con);
  ...
  if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['check_out'])) {
      // Check-out action
      $query = "INSERT INTO attendance_sheet (user_id, check_out, latitude_check_out, longitude_check_out, remark, check_out_time)
          VALUES (?, 'Yes', ?, ?, ?, NOW())";
      $stmt = mysqli_prepare($con, $query);
      mysqli_stmt_bind_param($stmt, "isss", $user_data['id'], $latitude_check_out, $longitude_check_out, $remark);
    } else {
      // Check-in action
      ...
      function getLocationAndSubmit(event, isCheckOut) {
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            function(position) {
              if (isCheckOut) {
                console.log("Check Out - Latitude: " + position.coords.latitude);
                console.log("Check Out - Longitude: " + position.coords.longitude);
                ...
                console.log("Check In - Not submitting latitude and longitude for Check In");
              }
          ...
          // Handle the lack of geolocation support or provide feedback to the user
        }
      }
    </script>
  </div>
